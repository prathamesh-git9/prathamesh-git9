...
| --- | --- | --- |
| [PatchPilot](https://github.com/prathamesh-git9/patchpilot) | Provider-authored OpenAPI changes should migrate affected customer code instead of becoming unread changelog entries. Built in response to YC's [Self-Maintaining APIs](https://www.ycombinator.com/rfs) request. | Provider campaigns fan signed releases only to explicitly consented customer repos; consent, expiry, and revocation are checked before token issuance, then each customer verifies its pinned key before deterministic JS/TS/Python edits become a PR. 124 tests, 15 releases, green CI. |
